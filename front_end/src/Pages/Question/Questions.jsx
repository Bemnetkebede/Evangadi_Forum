...
function Question() {
  ...
  useEffect(() => {
    ...
    if (token) {
      ...
      setLoading(false);
      // Handle cases where there's no token, such as redirecting the user to login
    }
  }, []);
...
const filteredQuestions = Array.isArray(questions)
  ? questions.filter((question) => {
      ...
      const contentMatches = question.content
        .toLowerCase()
        .includes(searchQuery.toLowerCase()); // Use 'content' instead of 'description'
      return titleMatches || contentMatches;
    })
  : []; // Return an empty array if questions is not an array
